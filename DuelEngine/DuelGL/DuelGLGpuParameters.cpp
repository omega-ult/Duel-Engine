...
// 
// 	void GLGpuTextureConstant::setSamplerState( const DTextureSampler& val )
// 	{
// 		if (!isValid())
// 		{
// 			return;
...
// 	}
	...
	void GLGpuTextureConstant::setSamplerObject( DTextureSamplerObjectPtr samp )
	{
		if (!isValid())
		{
			return;
		...
		if (samp != NULL)
		{
			glBindSampler(getTextureID(), samp->getAs<GLTextureSamplerObject>()->getSamplerObjectID());
		}
		else
		{
			glBindSampler(getTextureID(), 0);
		}
	}
